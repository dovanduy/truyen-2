                 ...
                 <div class="col-lg-12">
                    <div class="panel">
                        ...
                        <div class="panel-body">
                            <form class="form-horizontal" role="form" method="get" action="{{url('user/sms/historyFile')}}" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-xs-6">
                                    <input type="text" class="form-control createdDate" value="<?php echo isset($_GET['created_date'])?$_GET['created_date']:''?>" name="created_date" placeholder="Nhập ngày gửi">
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="selectpicker form-control" name="status" data-live-search="true">
                                            ...
                                            <option value="1" <?php if(isset($_GET['status']) && $_GET['status']==1) echo 'selected'?>>Thành công</option>
                                            <option value="2"  <?php if(isset($_GET['status']) && $_GET['status']==2) echo 'selected'?>>Thất bại</option>
                                            <option value="3"  <?php if(isset($_GET['status']) && $_GET['status']==3) echo 'selected'?>>Tất cả</option>
                                    </select>
                                </div>
                                </div>
								<div class="form-group">
								...
								<div class="form-group">
                                    <label class="control-label col-xs-2">Gửi theo danh bạ</label>
                                    <div class="col-xs-6">
                                    <select class="selectpicker form-control" name="gui_theo_danh_ba" data-live-search="true">
                                            <option value="">Chọn</option>
                                            <?php
                                                foreach ($gui_theo_danh_ba as $item) {
                                                    $selected="";
                                                    if(isset($_GET['gui_theo_danh_ba']) && $item->file_send==$_GET['gui_theo_danh_ba'])
                                                        $selected="selected";
                                                    echo "<option value='".$item->file_send."'".$selected
                                                    .">".$item->file_send."</option>";
                                                }
								...
								<div class="form-group">
                                    <label class="control-label col-xs-2">Gửi tin cho nhóm khách hàng</label>
                                    <div class="col-xs-6">
                                    <select class="selectpicker form-control" name="gui_tin_cho_nhom_kh" data-live-search="true">
                                            <option value="">Chọn</option>
                                            <?php
                                                foreach ($gui_tin_cho_nhom_kh as $item) {
                                                    $selected="";
                                                    if(isset($_GET['gui_tin_cho_nhom_kh']) && $item->file_send==$_GET['gui_tin_cho_nhom_kh'])
                                                        $selected="selected";
                                                    echo "<option value='".$item->file_send."'".$selected
                                                    .">".$item->file_send."</option>";
                                                }
								...
								<div class="form-group">
                                    <label class="control-label col-xs-2">Gửi hàng loạt từ file</label>
                                    <div class="col-xs-6">
                                    <select class="selectpicker form-control" name="gui_hang_loat_tu_file" data-live-search="true">
                                            <option value="">Chọn</option>
                                            <?php
                                                foreach ($gui_hang_loat_tu_file as $item) {
                                                    $selected="";
                                                    if(isset($_GET['gui_hang_loat_tu_file']) && $item->file_send==$_GET['gui_hang_loat_tu_file'])
                                                        $selected="selected";
                                                    echo "<option value='".$item->file_send."'".$selected
                                                    .">".$item->file_send."</option>";
                                                }
                                ...
                                <a href="{{url('user/sms/historyFile')}}"><button type="button" class="btn btn-danger"><i class="fa fa-plus"></i> Reset </button></a>
                            </div>
                            </form>
                        </div>
                    </div>
                        ...
                        <div class="panel-body p-none">
                            <table class="table data-table table-hover table-ultra-responsive">
                                ...
                                <tr>
                                    ...
                                    <th style="width: 35%;">{{language_data('Message')}}</th>
                                ...
                                @foreach($sms_history as $h)
                                    <tr>
                                        ...
                                        <td><?php
                                            if (strpos($h->status, 'Success') !== false) {
                                                echo 'Thành công';
                                            }else {
                                                echo 'Thất bại';
                                            }
                                        ?></td>
										<td>{{$h->file_send}}</td>
                                        <td data-label="Date"><p>{{get_datetime_format($h->created_at)}}</p></td>
        ...
        <script>
            jQuery(document).ready(function($) {
                var engine = new Bloodhound({
                    remote: {
                        url: _url+'/user/findMobile?q=%QUERY%',
                        wildcard: '%QUERY%'
                    },
